...
export class UsersGatewayInMemory implements IUsersGateway {
  constructor(private hashService: HashPassword) {}
  private usersList: User[] = [];

  async register(newUser: ICreateUserDto): Promise<void> {
    const userAlreadyExists = this.usersList.some(
      (userData) =>
  ...
  async findById(id: string): Promise<User> {
    const targetUser = this.usersList.find((user) =>
      user.id === id ? user : [],
    );
    return targetUser;
  }
  ...
  async deleteUser(user: loginDto): Promise<boolean> {
    const targetUser = this.usersList.find(
      (userFromUserList) => userFromUserList.username === user.username,
    );
    if (targetUser === null) {
    ...
    if (passwordVerify === false) {
      ...
      const newUserList = this.usersList.filter(
        (userFromUserList) =>
          userFromUserList.id != targetUser.id &&
          userFromUserList.username != targetUser.username &&
          userFromUserList.email != targetUser.email,
      );
